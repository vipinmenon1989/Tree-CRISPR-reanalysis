...
def get_sort_key(file_name):
    ...
    # Strip extension (e.g., 'ALKE_hits.csv' -> 'ALKE_hits')
    base_name = os.path.splitext(file_name)[0]

    # Extract the prefix before the first underscore (e.g., 'ALKE_hits' -> 'ALKE')
    # If there is no underscore, it just uses the base name (e.g., 'K')
    prefix = base_name.split("_")[0]

    # Define your exact required order of prefixes
    order_hierarchy = [
        "K",
        "ALKE",
        "ALKL",
    ...
    try:
        ...
        # Fallback for any unexpected files to keep the code from crashing
...
def analyze_gene_csvs(folder_path):
    ...
    # Sort files using the updated prefix-matching logic
