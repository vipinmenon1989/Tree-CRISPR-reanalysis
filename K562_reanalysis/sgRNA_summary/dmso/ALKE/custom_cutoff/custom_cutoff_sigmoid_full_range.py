...
def run_analysis(input_file, hits_output, dist_output):
    ...
    # 1. Define qualified sgRNAs (Sigmoid_Score >= 0.0 and <= 1.0)
    qualified_mask = (df['Sigmoid_Score'] >= 0.0) & (df['Sigmoid_Score'] <= 1.0)
    ...
    # 3. Create the "Hits" file (Both filters: Score >= 0.0 AND Count >= 2)
    ...
    # This removes sgRNAs that passed the score >= 0.0 but failed the count >= 2 filter
    dist_df = pd.concat([final_hits_df, ntc_df], axis=0)
    
    dist_df.to_csv(dist_output, index=False)

    print(f"Processing complete.")
    print(f"Full-range hits saved to: {hits_output}")
    print(f"Distribution file (Hits + NTC) saved to: {dist_output}")
    print(f"Total rows in distribution: {len(dist_df)}")

if __name__ == "__main__":
    if len(sys.argv) != 4:
        print("Usage: python custom_cutoff_sigmoid_full_range.py <input_csv_file> <hits_output_csv> <dist_output_csv>")
        sys.exit(1)
