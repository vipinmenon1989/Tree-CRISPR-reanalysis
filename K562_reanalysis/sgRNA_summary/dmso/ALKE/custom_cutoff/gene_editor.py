...
def main():
    ...
    # ======================================================================
    working_dir = "/local/projects-t3/lilab/vmenon/CRISPRi/Re-analysis/K562_reanalysis/sgRNA_summary/dmso/ALKE/custom_cutoff/"
    
    # Input CSV file name as specified
    input_file = os.path.join(working_dir, "ALKE_hits_isolated_equation.csv")
    ...
    df_grouped['Editor_ALKE'] = 1
    df_grouped['Editor_ALKH'] = 0
    df_grouped['Editor_ALKL'] = 0
    df_grouped['Editor_ALKME'] = 0
    df_grouped['Editor_ALKMH'] = 0
    df_grouped['Editor_ALKML'] = 0

    # Ensure columns follow your exact requested header positioning sequence
    target_hierarchy = [
        'Gene', 'mean_sigmoid_score', 'class', 
        'Editor_ALKE', 'Editor_ALKH', 'Editor_ALKL',
        'Editor_ALKME', 'Editor_ALKMH', 'Editor_ALKML'
    ]
