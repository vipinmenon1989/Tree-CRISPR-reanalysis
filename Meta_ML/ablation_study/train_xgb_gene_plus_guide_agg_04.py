...
def main():
    ...
    metrics_file = os.path.join(output_dir, "metrics_xgb_gene_plus_guide_agg_04.txt")
    feat_importance_file = os.path.join(output_dir, "xgb_gene_plus_guide_agg_04_feature_importance.csv")
    model_output_path = os.path.join(output_dir, "best_xgb_gene_plus_guide_agg_04_model.pkl")
    ...
    # --- 2. Feature Selection & Aggregation: Gene (RAW) + Guide (AGGREGATED) ---
    ...
    print(f"Gene (RAW) + Guide (AGG) feature matrix X dimensions: {X.shape}")
    ...
    print("Starting Gene + Guide AGG XGBoost training...")
    ...
    with open(metrics_file, 'w') as f:
        f.write("==================================================\n")
        f.write("XGBOOST ABLATION GENE (RAW) + GUIDE (AGG) (0.4) PERFORMANCE METRICS\n")
        f.write("==================================================\n")
