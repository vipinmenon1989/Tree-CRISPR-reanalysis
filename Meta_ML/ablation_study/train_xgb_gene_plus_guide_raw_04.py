...
def main():
    ...
    metrics_file = os.path.join(output_dir, "metrics_xgb_gene_plus_guide_raw_04.txt")
    feat_importance_file = os.path.join(output_dir, "xgb_gene_plus_guide_raw_04_feature_importance.csv")
    model_output_path = os.path.join(output_dir, "best_xgb_gene_plus_guide_raw_04_model.pkl")
    ...
    # --- 2. Feature Selection: Gene + Guide RAW (Keep all original bins raw) ---
    X = df.drop(columns=['ID', target_col])
    print(f"Gene + Guide RAW feature matrix X dimensions: {X.shape}")
    ...
    print("Starting Gene + Guide RAW XGBoost training...")
    ...
    with open(metrics_file, 'w') as f:
        f.write("==================================================\n")
        f.write("XGBOOST ABLATION GENE + GUIDE RAW (0.4) PERFORMANCE METRICS\n")
        f.write("==================================================\n")
