...
def main():
    ...
    metrics_file = os.path.join(output_dir, "metrics_xgb_baseline_04.txt")
    feat_importance_file = os.path.join(output_dir, "xgb_baseline_04_feature_importance.csv")
    model_output_path = os.path.join(output_dir, "best_xgb_baseline_04_model.pkl")
    ...
    # --- 2. Feature Selection: Baseline (Drop ALL _bin_ columns) ---
    bin_cols = [col for col in df.columns if '_bin_' in col]
    print(f"Purging {len(bin_cols)} epigenetic columns for Baseline study...")
    
    X = df.drop(columns=['ID', target_col] + bin_cols)
    print(f"Baseline feature matrix X dimensions: {X.shape}")
    ...
    print("Starting baseline XGBoost training...")
    ...
    with open(metrics_file, 'w') as f:
        f.write("==================================================\n")
        f.write("XGBOOST ABLATION BASELINE (0.4) PERFORMANCE METRICS\n")
        f.write("==================================================\n")
