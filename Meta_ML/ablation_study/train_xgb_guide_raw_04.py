...
def main():
    ...
    metrics_file = os.path.join(output_dir, "metrics_xgb_guide_raw_04.txt")
    feat_importance_file = os.path.join(output_dir, "xgb_guide_raw_04_feature_importance.csv")
    model_output_path = os.path.join(output_dir, "best_xgb_guide_raw_04_model.pkl")
    ...
    # --- 2. Feature Selection: Guide-Level RAW (Drop gene-level _bin_ columns, keep guide_ bin columns) ---
    gene_bin_cols = [col for col in df.columns if '_bin_' in col and not col.startswith('guide_')]
    print(f"Purging {len(gene_bin_cols)} gene-level raw epigenetic bin columns for Guide-level RAW study...")
    
    X = df.drop(columns=['ID', target_col] + gene_bin_cols)
    print(f"Guide RAW feature matrix X dimensions: {X.shape}")
    ...
    print("Starting Guide-level RAW XGBoost training...")
    ...
    with open(metrics_file, 'w') as f:
        f.write("==================================================\n")
        f.write("XGBOOST ABLATION GUIDE RAW (0.4) PERFORMANCE METRICS\n")
        f.write("==================================================\n")
