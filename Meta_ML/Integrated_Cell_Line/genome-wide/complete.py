...
def main():
    ...
    # 3. Enforce Row-Level Stratified 80/20 Internal Data Split
    ...
    # 4. XGBoost Small-Sample Regularization Engine
    ...
    # Restructured constraints specifically for your 505-row footprint
    param_grid = {
        'n_estimators': [30, 50, 75],            # Lower tree count blocks endless boosting loops
        'max_depth': [2, 3],                     # Force shallow structures to stop multi-feature combinations
        'learning_rate': [0.01, 0.02, 0.03],     # Slower step sizes maximize stable gradient descents
        'subsample': [0.6, 0.7],                 # Drop 30-40% of rows per tree to force generalization
        'colsample_bytree': [0.2, 0.3, 0.4],     # Aggressive column limits ensure weak trees evaluate separate markers
        'reg_alpha': [5, 10, 15],                # High L1 penalties zero out low-signal chromatin bins
        'reg_lambda': [10, 15, 20]               # Extreme L2 weights reduce variable tree prediction variances
    }
    
    cv = StratifiedKFold(n_splits=5, shuffle=True, random_state=42)
    print("[*] Tuning hyperparameters via constrained StratifiedKFold CV (30 iterations)...")
    ...
    with open(metrics_output_path, 'w') as f:
        f.write("=== Full Multi-Modality Model Metrics (Small Sample Cross-Guide Config) ===\n")
        ...
        for k, v in metrics.items():
            f.write(f"{k}: {v:.4f}\n")
        f.write("-" * 65 + "\n")
        f.write("\n=== Optimized Highly Regularized Hyperparameters ===\n")
    ...
    plt.title("Full Model Feature Importance (TreeSHAP Regularized)", fontsize=12, fontweight='bold', pad=15)
    plt.tight_layout()
    ...
    print(f"[-->] Cross-guide run complete. Logs written to: {metrics_output_path}")
