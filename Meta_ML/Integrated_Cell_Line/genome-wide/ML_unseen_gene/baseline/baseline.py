...
def main():
    ...
    print("[*] Ingesting 80% training master matrix...")
    ...
    # Isolate numeric sequence-only array features
    ...
    # This keeps specific genes entirely within either train or validation
    ...
    print(f"--> Internal Split Generated: Train Structure = {X_train.shape[0]} rows | Validation Structure = {X_val.shape[0]} rows")
    ...
    # Tune parameters using cross-validation within the training subset
    cv = GroupKFold(n_splits=5)
    print("[*] Tuning training hyperparameters via inner GroupKFold CV...")
    ...
    # 5. Evaluate Performance on the Internal 20% Validation Subset
    print("[*] Running inference on the internal 20% validation subset...")
    ...
    with open(metrics_output_path, 'w') as f:
        f.write("=== Sequence Feature Ablation Study Metrics (Internal 80/20 Split) ===\n")
        f.write(f"Internal Training Array Size: {X_train.shape[0]} rows\n")
        f.write(f"Internal Validation Array Size: {X_val.shape[0]} rows\n")
