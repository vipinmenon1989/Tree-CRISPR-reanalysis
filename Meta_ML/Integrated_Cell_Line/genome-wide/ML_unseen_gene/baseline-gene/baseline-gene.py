...
def main():
    ...
    # Clean Output Configurations
    metrics_output_path = os.path.join(working_dir, "sequence_plus_gene_epi_metrics.txt")
    curves_png_path = os.path.join(working_dir, "sequence_plus_gene_epi_curves.png")
    curves_pdf_path = os.path.join(working_dir, "sequence_plus_gene_epi_curves.pdf")
    shap_png_path = os.path.join(working_dir, "sequence_plus_gene_epi_shap.png")
    shap_pdf_path = os.path.join(working_dir, "sequence_plus_gene_epi_shap.pdf")
    
    if not os.path.exists(train_file):
        print(f"CRITICAL ERROR: Training file missing from {working_dir}")
        sys.exit(1)
        
    print("[*] Ingesting master matrix layout (~1400 rows)...")
    ...
    # STRATEGY: Drop local guide chromatin tracks, KEEP gene promoter bins
    guide_level_epigenetic_bins = [
    ...
    total_drops = list(set(metadata_drops + guide_level_epigenetic_bins))
    
    # Slice the clean dataset (Numeric sequence features + gene promoter bins)
    ...
    retained_gene_bins = [col for col in X_master.columns if 'bin' in col and not col.startswith('guide_')]
    print(f"--> Extraction Layer Diagnostics Complete.")
    print(f"    Total Matrix Width: {X_master.shape[1]} features.")
    print(f"    Retained Promoter Gene-Epi channels: {len(retained_gene_bins)}")
    ...
    with open(metrics_output_path, 'w') as f:
        f.write("=== Sequence + Gene Epigenetics Model Metrics (Internal 80/20 Split) ===\n")
    ...
    ax1.plot(fpr, tpr, color='darkorange', lw=2.5, label=f'Seq + Gene Epi Model (AUC = {roc_auc:.3f})')
    ...
    ax2.plot(recall, precision, color='dodgerblue', lw=2.5, label=f'Seq + Gene Epi Model (PR-AUC = {pr_auc:.3f})')
    ...
    plt.title("Sequence + Gene Epigenetic Importance (TreeSHAP Beeswarm)", fontsize=12, fontweight='bold', pad=15)
    plt.tight_layout()
