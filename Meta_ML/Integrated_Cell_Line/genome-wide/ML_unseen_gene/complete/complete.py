...
def main():
    ...
    # Comprehensive Full-Feature Model Output Files
    metrics_output_path = os.path.join(working_dir, "sequence_plus_all_epi_metrics.txt")
    curves_png_path = os.path.join(working_dir, "sequence_plus_all_epi_curves.png")
    curves_pdf_path = os.path.join(working_dir, "sequence_plus_all_epi_curves.pdf")
    shap_png_path = os.path.join(working_dir, "sequence_plus_all_epi_shap.png")
    shap_pdf_path = os.path.join(working_dir, "sequence_plus_all_epi_shap.pdf")
    
    if not os.path.exists(train_file):
        print(f"CRITICAL ERROR: Training master file missing from {working_dir}")
        sys.exit(1)
    ...
    # 2. Advanced Feature Isolation Layer (Preserves ALL Gene & Guide Epigenetics)
    explicit_metadata_drops = [
        'unique_sgrna_id', 'id', 'gene', 'sgrna sequence', 
        'cell_line_origin', 'distance_to_tss', 'sigmoid_score', 'class'
    ]
    
    # Strip tracking metadata handles, leaving ALL numeric indicators intact
    metadata_drops = [col for col in explicit_metadata_drops if col in df_master.columns]
    
    X_master = df_master.drop(columns=metadata_drops, errors='ignore').select_dtypes(include=[np.number])
    y_master = df_master['class'].values
    genes_master = df_master['gene'].values

    # Validation Diagnostic Audit
    total_retained_bins = [col for col in X_master.columns if 'bin' in col]
    guide_specific_bins = [col for col in X_master.columns if col.startswith('guide_')]
    gene_promoter_bins  = [col for col in total_retained_bins if col not in guide_specific_bins]
    
    print("\n" + "="*70)
    print("COMPREHENSIVE MULTI-MODALITY FEATURE ISOLATION DIAGNOSTICS")
    print("-"*70)
    print(f" -> Total Combined Features Utilized: {X_master.shape[1]}")
    print(f" -> Total Retained Chromatin/Epigenetic Bins: {len(total_retained_bins)}")
    print(f"    - Broad Gene Promoter-level Tracks:     {len(gene_promoter_bins)}")
    print(f"    - Local Guide Target-level Tracks:       {len(guide_specific_bins)}")
    print("="*70 + "\n")
    ...
    with open(metrics_output_path, 'w') as f:
        f.write("=== Sequence + Full Epigenetics Model Metrics (Internal 80/20 Split) ===\n")
        f.write(f"Internal Training Matrix Size: {X_train.shape[0]} rows\n")
        f.write(f"Internal Validation Matrix Size: {X_val.shape[0]} rows\n")
        f.write(f"Total Features Utilized (Seq + Gene + Guide): {X_train.shape[1]}\n")
        f.write("-" * 65 + "\n")
    ...
    ax1.plot(fpr, tpr, color='darkorange', lw=2.5, label=f'Full Modality Model (AUC = {roc_auc:.3f})')
    ...
    ax2.plot(recall, precision, color='dodgerblue', lw=2.5, label=f'Full Modality Model (PR-AUC = {pr_auc:.3f})')
    ...
    plt.title("Sequence + All Epigenetic Importance (TreeSHAP Beeswarm)", fontsize=12, fontweight='bold', pad=15)
    plt.tight_layout()
    ...
    print(f"[-->] Results generated successfully. Logs: {metrics_output_path}")
