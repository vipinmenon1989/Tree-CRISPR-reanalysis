...
def main():
    ...
    metrics_output_path = os.path.join(working_dir, "sequence_feature_ablation_metrics.txt")
    curves_png_path = os.path.join(working_dir, "sequence_evaluation_curves.png")
    curves_pdf_path = os.path.join(working_dir, "sequence_evaluation_curves.pdf")
    shap_png_path = os.path.join(working_dir, "sequence_shap_summary.png")
    shap_pdf_path = os.path.join(working_dir, "sequence_shap_summary.pdf")
    
    if not os.path.exists(train_file):
        print(f"CRITICAL ERROR: Data pool file missing from target directory: {train_file}")
        sys.exit(1)
        
    print("[*] Ingesting low-sample master matrix file...")
    ...
    print(f"--> Low-Sample Split: Train Pool = {X_train.shape[0]} rows | Validation Pool = {X_val.shape[0]} rows")
    ...
    # Restructured Grid designed strictly to suppress small-sample variance
    param_grid = {
        'n_estimators': [30, 50, 70],          # Capped tree count prevents boosting loops from memorizing noise
        'max_depth': [2, 3],                    # Extremely shallow limits interaction memorization
        'learning_rate': [0.01, 0.03],
        'subsample': [0.6, 0.7],                # Row subsampling preserves variation across chunks
        'colsample_bytree': [0.3, 0.5],         # Heavy feature restrictions force weak tree configurations
        'reg_alpha': [5, 10],                   # High L1 penalties drop low-signal features entirely
        'reg_lambda': [10, 15]                  # High L2 weights scale down noisy split assignments
    }
    
    cv = GroupKFold(n_splits=5)
    print("[*] Launching high-regularization GroupKFold CV (15 search iterations)...")
    random_search = RandomizedSearchCV(
        estimator=xgb_clf, 
        param_distributions=param_grid, 
        n_iter=15,                              # Reduced iterations to lower search-space exploitation
        scoring='roc_auc', 
        cv=cv, 
        n_jobs=-1, 
        random_state=42
    ...
    # 5. Evaluate Performance against the Unseen Gene Split
    print("[*] Running inference on low-sample validation pool...")
    ...
    with open(metrics_output_path, 'w') as f:
        f.write("=== Sequence Feature Ablation Study Metrics (Low Sample Config) ===\n")
    ...
    print("[*] Low-sample script processing complete.")
