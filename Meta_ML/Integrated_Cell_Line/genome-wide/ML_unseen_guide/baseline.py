...
def main():
    ...
    # Updated Output File Schemas for Cross-Guide Run
    metrics_output_path = os.path.join(working_dir, "cross_guide_sequence_metrics.txt")
    curves_png_path = os.path.join(working_dir, "cross_guide_sequence_curves.png")
    curves_pdf_path = os.path.join(working_dir, "cross_guide_sequence_curves.pdf")
    shap_png_path = os.path.join(working_dir, "cross_guide_sequence_shap.png")
    shap_pdf_path = os.path.join(working_dir, "cross_guide_sequence_shap.pdf")
    ...
    # Swapped out GroupKFold for StratifiedKFold to support cross-guide tuning
    ...
    with open(metrics_output_path, 'w') as f:
        f.write("=== Sequence Feature Ablation Study Metrics (Stratified Cross-Guide Split) ===\n")
    ...
    ax1.plot(fpr, tpr, color='darkorange', lw=2.5, label=f'Cross-Guide Model (AUC = {roc_auc:.3f})')
    ...
    ax2.plot(recall, precision, color='dodgerblue', lw=2.5, label=f'Cross-Guide Model (PR-AUC = {pr_auc:.3f})')
    ...
    plt.title("Cross-Guide Sequence Importance (TreeSHAP Beeswarm)", fontsize=12, fontweight='bold', pad=15)
    plt.tight_layout()
    ...
    print(f"[-->] Cross-guide metrics written to: {metrics_output_path}")
