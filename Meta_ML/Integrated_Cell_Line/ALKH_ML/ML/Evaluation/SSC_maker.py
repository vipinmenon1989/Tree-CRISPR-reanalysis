...
def merge_ssc_data():
    working_dir = "./"
    holdout_path = os.path.join(working_dir, "CGD_evaluation_CRISPRi.txt")
    ...
    # === THE FIX: DEDUPLICATE THE SSC REFERENCE DICTIONARY ===
    # Keep only the first instance of each sequence to prevent merge explosions
    df_ssc = df_ssc.drop_duplicates(subset=[ssc_seq_col], keep='first')
    ...
    print(f"[*] Final merged dimensions: {df_merged.shape[0]} rows")
    ...
    print(f"[✔] Success! Saved matches to {output_path}")
