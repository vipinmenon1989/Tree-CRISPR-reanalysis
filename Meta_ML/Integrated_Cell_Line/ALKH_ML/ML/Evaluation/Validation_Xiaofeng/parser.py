...
# 1. Load data
df = pd.read_csv("merged_sgRNA_data.txt", sep='\t')
df['gene'] = df['gene'].str.strip()

# 2. Keep only exact duplicate rows (Prevents losing target position/replicate variations)
# Removing the 'subset' parameter ensures we only drop 100% identical rows.
clean_df = df.drop_duplicates()

# 3. Filter for genes with 2+ guides (Ensures biological robustness)
valid_genes = clean_df.groupby('gene').filter(lambda x: len(x) >= 2).copy()
...
final_set = final_set.sort_values(by=['gene_status', 'gene', 'unique_sgrna_id'])

# 6. Save the comprehensive file with ALL columns intact
final_set.to_csv("Comprehensive_prediction_meta_file.txt", sep='\t', index=False)

print(f"--- COMPREHENSIVE DATASET EXTRACTED ---")
print(f"Total Genes: {final_set['gene'].nunique()}")
print(f"Total Guides (Rows): {len(final_set)}")
