...
def main():
    ...
    working_dir = "/local/projects-t3/lilab/vmenon/CRISPRi/Re-analysis/Meta_ML/Integrated_Cell_Line/Isolated_equation/unseen_guide/"
    train_file = os.path.join(working_dir, "CRISPRi_ML_Train_80_unseen_guides.txt")
    
    # Path configurations explicitly named to reflect the unseen guide baseline sequence run
    model_output_path = os.path.join(working_dir, "unseen_guide_baseline_model.json")
    metrics_output_path = os.path.join(working_dir, "unseen_guide_baseline_metrics.txt")
    curves_png_path = os.path.join(working_dir, "unseen_guide_baseline_curves.png")
    curves_pdf_path = os.path.join(working_dir, "unseen_guide_baseline_curves.pdf")
    shap_png_path = os.path.join(working_dir, "unseen_guide_baseline_shap.png")
    shap_pdf_path = os.path.join(working_dir, "unseen_guide_baseline_shap.pdf")
    ...
    # ==================================================================
    print(f"--> Saving final optimized unseen guide baseline booster model file to: {model_output_path}")
    best_model.save_model(model_output_path)

    print(f"--> Exporting comprehensive evaluation log to: {metrics_output_path}")
    with open(metrics_output_path, 'w') as f:
        f.write("=== Baseline Sequence Master Model Metrics (Unseen Guide Split) ===\n")
    ...
    print(f"[-->] Unseen guide baseline script completed successfully inside {working_dir}")
