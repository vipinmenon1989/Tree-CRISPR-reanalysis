...
def main():
    ...
    working_dir = "/local/projects-t3/lilab/vmenon/CRISPRi/Re-analysis/Meta_ML/Integrated_Cell_Line/Isolated_equation/random/evaluation/"
    
    # Define your new evaluation dataset filename here
    eval_file = os.path.join(working_dir, "CRISPRi_ML_Holdout_Test_20_random.txt") 
    model_json_path = os.path.join(working_dir, "full_multimodality_model.json")
    
    # Distinct output paths to prevent overwriting your training records
    metrics_output_path = os.path.join(working_dir, "external_evaluation_metrics.txt")
    curves_png_path = os.path.join(working_dir, "external_evaluation_curves.png")
    curves_pdf_path = os.path.join(working_dir, "external_evaluation_curves.pdf")
    ...
    if not os.path.exists(eval_file):
        print(f"CRITICAL ERROR: Evaluation dataset missing at: {eval_file}")
        sys.exit(1)

    # 2. Load and Reconstruct Saved XGBoost Architecture
    print("[*] Reconstituting saved full multi-modality model from JSON...")
    model = xgb.XGBClassifier()
    model.load_model(model_json_path)
    
    # Extract the exact feature names and order the model was trained on
    ...
    df_eval = pd.read_csv(eval_file, sep=None, engine='python')
    ...
    try:
        ...
        print(f"CRITICAL ERROR: Evaluation file is missing {len(missing_cols)} features required by the model!")
        print(f"First 5 missing features: {missing_cols[:5]}")
        sys.exit(1)
    ...
    with open(metrics_output_path, 'w') as f:
        f.write("=== External Dataset Evaluation Metrics Summary ===\n")
