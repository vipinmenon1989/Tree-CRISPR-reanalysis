...
def main():
    ...
    working_dir = "/local/projects-t3/lilab/vmenon/CRISPRi/Re-analysis/Meta_ML/Integrated_Cell_Line/Isolated_equation/random"    
    train_file = os.path.join(working_dir, "CRISPRi_ML_Train_80_random.txt")
    
    # Updated output paths tracking configurations
    model_output_path = os.path.join(working_dir, "baseline_sequence_model.json")
    metrics_output_path = os.path.join(working_dir, "random_split_baseline_metrics.txt")
    curves_png_path = os.path.join(working_dir, "random_split_baseline_curves.png")
    curves_pdf_path = os.path.join(working_dir, "random_split_baseline_curves.pdf")
    shap_png_path = os.path.join(working_dir, "random_split_baseline_shap.png")
    shap_pdf_path = os.path.join(working_dir, "random_split_baseline_shap.pdf")
    ...
    # Tuning grid calibrated for 2,103 raw rows
    param_grid = {
    ...
    with open(metrics_output_path, 'w') as f:
        f.write("=== Baseline Sequence Master Model Metrics (Randomized Split) ===\n")
    ...
    print(f"[-->] Baseline randomized script completed inside {working_dir}")
