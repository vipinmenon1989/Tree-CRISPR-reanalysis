...
def main():
    # 1. Cluster Workspace Configuration (Targeting A375 Locus Workspace)
    working_dir = "/local/projects-t3/lilab/vmenon/CRISPRi/Re-analysis/Meta_ML/Integrated_Cell_Line/Isolated_equation/cell_line/A375"    
    train_file = os.path.join(working_dir, "CRISPR_ml_features_final_A375.csv")
    
    # Path configuration for model checkpoint and evaluation outputs (full multi-modality unseen gene)
    model_output_path = os.path.join(working_dir, "unseen_gene_multimodality_model_720.json")
    metrics_output_path = os.path.join(working_dir, "unseen_gene_multimodality_metrics_720.txt")
    curves_png_path = os.path.join(working_dir, "unseen_gene_multimodality_curves_720.png")
    curves_pdf_path = os.path.join(working_dir, "unseen_gene_multimodality_curves_720.pdf")
    shap_png_path = os.path.join(working_dir, "unseen_gene_multimodality_shap_720.png")
    shap_pdf_path = os.path.join(working_dir, "unseen_gene_multimodality_shap_720.pdf")
    ...
    # Isolate all numeric features; sequence layers and all binned chromatin tracks are fully active
    ...
    # Constrained hyperparameter tuning parameters to prevent overfitting 751 features on small row volume
    param_grid = {
    ...
    with open(metrics_output_path, 'w') as f:
        f.write("=== Full Multi-Modality Model Metrics (Unseen Gene Split - 720 Rows) ===\n")
    ...
    print(f"[-->] Full multimodality unseen gene script completed inside {working_dir}")
