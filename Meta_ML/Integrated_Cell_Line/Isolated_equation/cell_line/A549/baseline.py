...
def main():
    # 1. Cluster Workspace Configuration (Targeting A549 Locus Workspace)
    working_dir = "/local/projects-t3/lilab/vmenon/CRISPRi/Re-analysis/Meta_ML/Integrated_Cell_Line/Isolated_equation/cell_line/A549"    
    train_file = os.path.join(working_dir, "CRISPR_ml_features_final_A549.csv")
    
    # Path configuration for model checkpoint and evaluation outputs (unseen gene baseline)
    model_output_path = os.path.join(working_dir, "unseen_gene_baseline_model_720.json")
    metrics_output_path = os.path.join(working_dir, "unseen_gene_baseline_metrics_720.txt")
    curves_png_path = os.path.join(working_dir, "unseen_gene_baseline_curves_720.png")
    curves_pdf_path = os.path.join(working_dir, "unseen_gene_baseline_curves_720.pdf")
    shap_png_path = os.path.join(working_dir, "unseen_gene_baseline_shap_720.png")
    shap_pdf_path = os.path.join(working_dir, "unseen_gene_baseline_shap_720.pdf")
    ...
    with open(metrics_output_path, 'w') as f:
        f.write("=== Baseline Sequence Model Metrics (Unseen Gene Split - 720 Rows) ===\n")
    ...
    print(f"[-->] Baseline unseen gene script completed inside {working_dir}")
