...
def main():
    ...
    # ==================================================================
    # [NEW] MEASURE CLASS DISTRIBUTION AND IMBALANCE
    # ==================================================================
    class_0_total = (df_master['class'] == 0).sum()
    class_1_total = (df_master['class'] == 1).sum()
    total_rows = len(df_master)
    imbalance_ratio = class_0_total / class_1_total if class_1_total > 0 else float('inf')
    
    print("[*] Target Class Distribution (Master Matrix):")
    print(f"    -> Class 0 (Negative): {class_0_total} ({(class_0_total/total_rows)*100:.2f}%)")
    print(f"    -> Class 1 (Positive): {class_1_total} ({(class_1_total/total_rows)*100:.2f}%)")
    print(f"    -> Imbalance Ratio: 1 positive sample for every {imbalance_ratio:.2f} negative samples.")
    # ==================================================================
    ...
    explicit_metadata_drops = [
        ...
        'cell_line_origin', 'distance_to_tss', 'sigmoid_score', 'class','start','end','closest_tss_coord'
    ]
    ...
    with open(metrics_output_path, 'w') as f:
        ...
        f.write(f"Class 0 (Negative): {class_0_total} ({(class_0_total/total_rows)*100:.2f}%)\n")
        f.write(f"Class 1 (Positive): {class_1_total} ({(class_1_total/total_rows)*100:.2f}%)\n")
    ...
    plt.title("Sequence + Local Guide Epigenetic Feature Importance (TreeSHAP Beeswarm)", fontsize=12, fontweight='bold', pad=15)
    plt.tight_layout()
    ...
    print(f"[-->] Script configuration completed successfully inside {working_dir}")
