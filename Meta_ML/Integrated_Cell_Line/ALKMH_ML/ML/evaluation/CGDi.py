...
def cgdscoring(input_file, output_prefix, threshold=0.0):
    ...
    if 'extended_sequence' not in df.columns:
        print("CRITICAL ERROR: 'extended_sequence' (30nt context) missing from input file header!")
        sys.exit(1)
    ...
    # Re-align headers to match the new requested output delivery
    df_output = pd.DataFrame({
        'unique_sgrna_id': df['unique_sgrna_id'].values,
        'id': df['id'].values,
        'sgrna sequence': df['sgrna sequence'].values,
        'gene': df['gene'].values,
        'distance_to_tss': df['distance_to_tss'].values,
        'strand': df['strand'].values,
        'chromosome': df['chromosome'].values,
        'extended_sequence': df['extended_sequence'].values,
        'sigmoid_score': df['sigmoid_score'].values,
        'class': df['class'].values,
        'CGDi_Score': df['cgdi_score'].values
    })

    # Save target independent predictions matrix
    scored_file = os.path.join(os.path.dirname(input_file), f"{output_prefix}_independent_predictions.txt")
...
if __name__ == "__main__":
    ...
    # Set the default threshold explicitly to 0.0 per instructions
