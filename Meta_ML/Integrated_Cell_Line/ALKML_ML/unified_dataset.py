...
def harmonize_and_combine_matrices(k562_path, a549_path, a375_path, combined_output_path):
    print("======================================================================")
    print("[*] CRISPRi TRIPLE-CELL INTEGRATION ENGINE (v3.1 - PREFIX STRIP & CLASS GEN)")
    print("======================================================================\n")
    ...
    def load_and_normalize(cell_name, path):
        print(f"[*] Parsing and stripping cell prefixes from {cell_name} matrix...")
        ...
        if 'sigmoid_score' not in df.columns:
            print(f"CRITICAL ERROR: 'sigmoid_score' is missing from {cell_name}. Cannot generate 'class'.")
            sys.exit(1)
    ...
    mandatory_keys = ['id', 'sigmoid_score', 'distance_to_tss', 'class', 'strand', 'sgrna sequence', 'extended_sequence', 'chromosome']
...
if __name__ == "__main__":
    harmonize_and_combine_matrices(
        k562_path='CRISPR_ml_features_final_ALKML_K562.csv',
        a549_path='CRISPR_ml_features_final_ALKML_A549.csv',
        a375_path='CRISPR_ml_features_final_ALKML_A375.csv',
