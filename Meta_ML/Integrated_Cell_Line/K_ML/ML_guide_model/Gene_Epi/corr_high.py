...
file_path = "Guide_Level_Aggregated_Epigenetics.csv"
print("Reading essential gene feature matrix...")
df = pd.read_csv(file_path, sep=",")

# 3. CRITICAL SUBSETTING: Isolate highly active genes (Score > 0.5)
print("Filtering rows where mean_sigmoid_score > 0.5...")
df_high = df[df["mean_sigmoid_score"] > 0.7].reset_index(drop=True)
...
print("==================================================")
print(f"Total Active Genes Checked (>0.5)  : {len(df_high)}")
...
report_output = "essential_genes_high_score_correlations_guide.txt"
...
plt.title("Essential Genes Landscape Profile (All Spatial Feature Tracks Checked | Score > 0.5)\n* Bolded Cell Entry Highlights Statistically Significant Window (p <= 0.05)", 
          fontsize=11, fontweight="bold", pad=20)

plt.tight_layout()

# 10. Save plots in dual cluster-ready graphics formats
png_path = os.path.join("plots", "essential_70_row_landscape_heatmap_guide.png")
pdf_path = os.path.join("plots", "essential_70_row_landscape_heatmap_guide.pdf")
