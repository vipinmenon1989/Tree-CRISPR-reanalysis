...
def main():
    working_dir = "/local/projects-t3/lilab/vmenon/CRISPRi/Re-analysis/Meta_ML/Integrated_Cell_Line/ablation_study_with_scaled_epigenetics"
    input_file = os.path.join(working_dir, "CRISPR_merged_file.csv")
    
    # Output paths
    model_output_path = os.path.join(working_dir, "full_model_xgboost_model.json")
    metrics_output_path = os.path.join(working_dir, "full_model_metrics.txt")
    shap_summary_plot_path = os.path.join(working_dir, "full_model_shap_summary.png")
    shap_bar_plot_path = os.path.join(working_dir, "full_model_shap_bar.png")
    ...
    # Feature Selection: Baseline Sequence + Guide Epi + Gene Epi
    cell_line_specific_cols = [col for col in df.columns if 'K562' in col]
    ...
    cols_to_drop = list(set(cell_line_specific_cols + metadata_cols))
    
    X = df.drop(columns=[col for col in cols_to_drop if col in df.columns]).select_dtypes(include=[np.number])
    y = df['class']
    
    print(f"Total features retained for Full Model: {X.shape[1]}")
    ...
    print("Executing Deep Search on Full Feature Model...")
    ...
    with open(metrics_output_path, 'w') as f:
        f.write("=== Full Feature Model Metrics ===\n")
    ...
    print("\n" + "="*30)
    print("Full Feature Model Metrics")
    print("="*30)
    for k, v in metrics.items():
        print(f"{k}: {v:.4f}")
   ...
   # --- SHAP EXPLAINER BLOCK ---
    ...
    plt.title("SHAP Global Feature Importance (Beeswarm Plot)", fontsize=14, pad=20)
    plt.tight_layout()
...
# 2. Generate and save the Bar Plot (Mean Absolute SHAP)
    ...
    # the creation or calculation of the "Sum of other features" bar.
    top_20_shap_values = shap_values[:, :20]
    
    # Plot the sliced object. max_display must match the sliced dimension exactly.
    shap.plots.bar(top_20_shap_values, max_display=20, show=False)
    
    # Simple, clean axis adjustments without deleting elements
    ax = plt.gca()
    ...
    shap_csv_path = os.path.join(working_dir, "full_model_all_feature_importances.csv")
