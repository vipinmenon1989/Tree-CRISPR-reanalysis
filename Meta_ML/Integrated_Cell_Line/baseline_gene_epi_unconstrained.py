...
def main():
    ...
    model_output_path = os.path.join(working_dir, "stage2_gene_epi_xgboost_model.json")
    metrics_output_path = os.path.join(working_dir, "stage2_gene_epi_metrics.txt")
    ...
    # Feature Selection: Sequence + GENE Epigenetics
    ...
    # 2. STRICTLY Drop GUIDE-level epigenetics (leaving Sequence + Gene-level Epi)
    guide_epi_cols = [col for col in df.columns if col.startswith('guide_')]
    
    # Combine and drop
    cols_to_drop = list(set(metadata_cols + guide_epi_cols))
    
    X = df.drop(columns=[col for col in cols_to_drop if col in df.columns]).select_dtypes(include=[np.number])
    y = df['class']
    
    print(f"Total features remaining (Sequence + Gene Epi): {X.shape[1]}")
    ...
    # Unconstrained parameter grid (max_depth restricted to lower bounds to prevent high-dimensional memorization)
    ...
    print("Executing Deep Search on Stage 2 (Gene Epigenetics)...")
    ...
    with open(metrics_output_path, 'w') as f:
        f.write("=== Stage 2: Gene Epigenetics Metrics ===\n")
    ...
    print("\n" + "="*35)
    print("Stage 2: Gene Epigenetics Metrics")
    print("="*35)
    for k, v in metrics.items():
        print(f"{k}: {v:.4f}")
