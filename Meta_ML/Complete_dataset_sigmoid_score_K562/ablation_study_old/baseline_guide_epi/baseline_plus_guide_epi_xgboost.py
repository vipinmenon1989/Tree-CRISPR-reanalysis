...
def main():
    ...
    model_output_path = os.path.join(working_dir, "guide_epi_xgboost_model.json")
    ...
    # 5. Strict feature selection logic for ABLATION STEP 3 (Baseline + Guide-level Epigenetics)
    # EXCLUDE all gene-level epigenetic features. Programmatically drop any columns that contain 'K562' or 'H3K' BUT DO NOT contain 'guide_'.
    cols_to_drop_epi = [col for col in df.columns if ('K562' in col or 'H3K' in col) and 'guide_' not in col]
    ...
    cols_to_drop = list(set(cols_to_drop_epi + [col for col in metadata_cols if col in df.columns]))
    X_temp = df.drop(columns=cols_to_drop, errors='ignore')
    
    # Retain the baseline features AND the guide-level epigenetic features (columns explicitly starting with 'guide_')
    ...
    for col in X_temp.columns:
        ...
        # Retain if it is a guide-level epigenetic feature
        elif col.startswith('guide_'):
            feature_cols.append(col)
    ...
    print(f"Baseline + Guide-level Epigenetics feature space shape: {X.shape}")
    ...
    # Save the trained best model to disk as guide_epi_xgboost_model.json
    ...
    # Write calculated metrics and best parameters to guide_epi_metrics.txt
    print(f"Writing metrics to: {metrics_output_path}")
    with open(metrics_output_path, 'w') as f:
        f.write("=== Baseline + Guide-Level Epigenetic XGBoost Model Evaluation (Ablation Step 3) ===\n")
