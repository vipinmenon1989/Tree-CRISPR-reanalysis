...
# SCRIPT 3 Feature Logic
gene_cols = [col for col in X_base.columns if ('K562' in col or 'H3K' in col) and 'guide_' not in col]
X = X_base.drop(columns=gene_cols)
print(f"Feature space size (Baseline + Guide Epi): {X.shape[1]}")
...
# Save the model
best_model.save_model("reg_3_baseline_guide_model.json")
...
with open("reg_3_baseline_guide_metrics.txt", "w") as f:
    f.write("=== Baseline + Guide-Level Epigenetic Regressor Metrics ===\n")
