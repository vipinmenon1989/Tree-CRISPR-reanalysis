...
# SCRIPT 2 Feature Logic
guide_cols = [col for col in X_base.columns if 'guide_' in col]
X = X_base.drop(columns=guide_cols)
print(f"Feature space size (Baseline + Gene Epi): {X.shape[1]}")
...
# Save the model
best_model.save_model("reg_2_baseline_gene_model.json")
...
with open("reg_2_baseline_gene_metrics.txt", "w") as f:
    f.write("=== Baseline + Gene-Level Epigenetic Regressor Metrics ===\n")
