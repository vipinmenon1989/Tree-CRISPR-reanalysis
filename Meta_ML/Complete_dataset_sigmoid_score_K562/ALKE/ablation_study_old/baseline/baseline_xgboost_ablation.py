...
def main():
    ...
    # 5. Strict feature selection logic for the BASELINE model (Ablation Step 1)
    # EXCLUDE all gene-based and guide-based epigenetic features
    # Programmatically drop any columns containing substrings 'K562', 'H3K', or 'guide_'
    ...
    # Retain only the baseline features: distance_to_TSS, thermodynamic features, and sequence features (count_*, pos_*, di_*)
    baseline_prefixes = ('count_', 'pos_', 'di_')
    baseline_exacts = {'distance_to_TSS', 'MFE', 'Tm', 'entropy', 'GC_count', 'GC_content', 'gc_low', 'gc_high'}
    
    feature_cols = [col for col in X_temp.columns if col in baseline_exacts or col.startswith(baseline_prefixes)]
    ...
    print(f"Baseline feature space shape: {X.shape}")
    ...
    # Define a highly regularized hyperparameter grid
    param_grid = {
    ...
    # Save the trained best model to disk as baseline_xgboost_model.json
    ...
    # Write calculated metrics and best parameters to baseline_metrics.txt
    print(f"Writing metrics to: {metrics_output_path}")
    with open(metrics_output_path, 'w') as f:
        f.write("=== Baseline XGBoost Model Evaluation (Ablation Step 1) ===\n")
