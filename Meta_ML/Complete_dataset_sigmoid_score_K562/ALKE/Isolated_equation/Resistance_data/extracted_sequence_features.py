...
def main():
    input_file = "ALKE_K562_epigenetic_training_resistance.txt"
    ...
    # Clean headers immediately to handle column case shifting across cluster setups
    df.columns = [str(col).strip() for col in df.columns]

    # Check for direct ground-truth class existence
    if 'class' not in df.columns:
        raise KeyError("Error: 'class' column is missing from the input file. Ensure File 3 is generated correctly first.")
    ...
    # ==================================================================
    # Structural metadata tracker list (Sigmoid_Score stripped completely)
    keep_cols = ['ID', 'sgRNA Sequence', 'Gene', 'distance_to_TSS', 'class']
