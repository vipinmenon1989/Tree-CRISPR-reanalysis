...
def main():
    ...
    # Path configuration for unique independent random split outputs
    model_output_path = os.path.join(working_dir, "random_split_multiomic_model.json")
    metrics_output_path = os.path.join(working_dir, "random_split_multiomic_metrics.txt")
    curves_png_path = os.path.join(working_dir, "random_split_multiomic_curves.png")
    curves_pdf_path = os.path.join(working_dir, "random_split_multiomic_curves.pdf")
    shap_png_path = os.path.join(working_dir, "random_split_multiomic_shap.png")
    shap_pdf_path = os.path.join(working_dir, "random_split_multiomic_shap.pdf")
    ...
    # ==================================================================
    # 3. ENFORCE STANDARD STRATIFIED RANDOM INDEPENDENT SPLIT (80/20)
    # ==================================================================
    # Fixed Implementation: Utilizing train_test_split with explicit class stratification
    ...
    print(f"--> Independent Random Split Layout: Train = {X_train.shape[0]} rows | Validation = {X_val.shape[0]} rows")
    ...
    # 6. Evaluate Performance on Out-Of-Sample Random Partition
    print("[*] Running inference on the random validation partition...")
    ...
    with open(metrics_output_path, 'w') as f:
        f.write("=== Multi-Omic Sequence + Epigenetic Model Metrics (Independent Random Split) ===\n")
        f.write(f"Random Training Pool Size: {X_train.shape[0]} rows\n")
        f.write(f"Random Validation Pool Size: {X_val.shape[0]} rows\n")
    ...
    print(f"[-->] Random split script complete. Performance profiles serialized inside: {working_dir}")
