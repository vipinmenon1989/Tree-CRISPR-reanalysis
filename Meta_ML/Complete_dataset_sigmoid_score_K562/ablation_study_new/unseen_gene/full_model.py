...
def main():
    working_dir = "/local/projects-t3/lilab/vmenon/CRISPRi/Re-analysis/Meta_ML/Complete_dataset_sigmoid_score_K562/ablation_study_new/unseen_gene/"
    ...
    # Target definition matches your baked-in target column rules
    if 'class' not in df.columns:
        df['class'] = (df['Sigmoid_Score'] > 0.25).astype(int)
    
    y = df['class']
    
    # Confirm structural gene target handle is present to drive the group-split engine
    if 'Gene' not in df.columns:
        print("CRITICAL ERROR: 'Gene' column is missing from features file.")
        sys.exit(1)
    ...
    # The Gene column is dropped from X but retained in df to orchestrate the split groups.
    metadata_cols = [
    ...
    # Diagnostics to verify feature integrity
    ...
    # ==================================================================
    # STERILE CROSS-GENE GROUP PARTITIONING (80/20 SPLIT)
    # ==================================================================
    # This guarantees that the test pool evaluates entirely UNSEEN GENES
    gss = GroupShuffleSplit(n_splits=1, test_size=0.2, random_state=42)
    train_idx, test_idx = next(gss.split(X, y, groups=df['Gene']))
    
    X_train, X_test = X.iloc[train_idx], X.iloc[test_idx]
    y_train, y_test = y.iloc[train_idx], y.iloc[test_idx]
    
    print(f"--> [TRAIN POOL] Rows: {X_train.shape[0]} | Unique Genes: {df.iloc[train_idx]['Gene'].nunique()}")
    print(f"--> [TEST POOL]  Rows: {X_test.shape[0]} | Unique Genes: {df.iloc[test_idx]['Gene'].nunique()}")
    ...
    with open(metrics_output_path, 'w') as f:
        ...
        f.write(f"Data Partition Layout: GroupShuffleSplit by Gene (Unseen Gene Validation)\n")
        f.write(f"Total Unique Evaluation Genes: {df.iloc[test_idx]['Gene'].nunique()}\n")
