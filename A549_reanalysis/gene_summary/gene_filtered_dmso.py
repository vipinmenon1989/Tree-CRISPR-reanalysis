...
def generate_a549_essentiality_plots():
    # 1. Path to your A375 specific filtered data
    base_dir = "TreeCRISPR_A549_Analysis/Filtered"
    
    if not os.path.exists(base_dir):
        print(f"Error: {base_dir} does not exist. Ensure the A549 pipeline has run.")
        return
    ...
    for editor in editors:
        ...
        filename = f"A549_{editor}_filtered.csv"
        ...
        try:
            ...
            if not plot_df.empty:
                ...
                plt.title(f'A549: Top 15 Essential Genes ({editor})', fontsize=14, fontweight='bold')
            ...
            plt.savefig(os.path.join(editor_path, f"A549_{editor}_top_hits.png"))
            plt.close()
            
            print(f"Finalized A549 Analysis for {editor}")
    ...
    summary_path = os.path.join(base_dir, "A549_Essentiality_Filtering_Summary.csv")
    df_summary.to_csv(summary_path, index=False)
    
    print("-" * 30)
    print(f"A549 SUMMARY SAVED TO: {summary_path}")
    print(df_summary.to_string(index=False))

if __name__ == "__main__":
    generate_a549_essentiality_plots()
