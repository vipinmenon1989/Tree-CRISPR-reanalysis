...
def run_a549_dropout_pipeline():
    input_dir = "./" 
    base_output_dir = "TreeCRISPR_A549_Analysis"
    ...
    for editor in editors:
        ...
        if not editor_files:
            print(f"Warning: No A549 summary file found for {editor}")
            continue
        ...
        try:
            ...
            df_unf.to_csv(os.path.join(base_output_dir, 'Unfiltered', editor, f'A549_{editor}_unfiltered.csv'), index=False)
            df_fil.to_csv(os.path.join(base_output_dir, 'Filtered', editor, f'A549_{editor}_filtered.csv'), index=False)
            ...
            plt.title(f'A549 {editor}: Dropout Essentiality')
            plt.xlabel('Log Fold Change (d15 vs d0)')
            plt.legend()
            
            plot_dir = os.path.join(base_output_dir, "Plots")
            os.makedirs(plot_dir, exist_ok=True)
            plt.savefig(os.path.join(plot_dir, f"A549_{editor}_dropout_dist.png"))
            plt.close()
    ...
    # Save Summary CSV
    pd.DataFrame(summary_data).to_csv(os.path.join(base_output_dir, "A549_Processing_Summary.csv"), index=False)
    print("A549 Pipeline Complete.")

if __name__ == "__main__":
    run_a549_dropout_pipeline()
